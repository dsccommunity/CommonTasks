...
:abstract:    {YmlCategory} manages Users, Computers, and Managed Service Accounts within Active Directory.
...
****
Use this resource when you need to create or manage AD user accounts, computer accounts, KDS root keys, or group/standalone Managed Service Accounts (gMSA/sMSA). This is the right choice for provisioning service identities and machine accounts in bulk as part of your infrastructure-as-code pipeline.
****
...
.Ordering: depend on AddsOrgUnitsAndGroups via its completion anchor
[NOTE]
====
When this composite runs together with <<dscyml_addsorgunitsandgroups>> on the same
node, order it *after* all OUs and groups by depending on the
`AddsOrgUnitsAndGroupsComplete` anchor — not on the whole `AddsOrgUnitsAndGroups`
composite:
...
Depending on the whole composite reference causes an O(users × (OUs + groups))
`DependsOn` explosion that can push the node MOF past the ~10 MB LCM limit
(`MOF file size exceeded max size limit` / `MI RESULT 27`). See
<<dscyml_addsorgunitsandgroups>> for details.
====
