...
:abstract:    {YmlCategory} manages Windows scheduled tasks, including creating, deleting, or disabling tasks on the local computer.
...
****
Use this resource when you need to create automated scheduled tasks (e.g., maintenance scripts, backup jobs, cleanup routines) or disable/delete unnecessary built-in scheduled tasks for security hardening. It supports one-time, recurring, and complex trigger-based task scheduling.
****
