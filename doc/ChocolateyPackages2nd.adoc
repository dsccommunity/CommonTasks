...
:abstract:    {YmlCategory} installs a second set of software packages using Chocolatey.
...
****
Use this resource when you have Chocolatey packages that depend on packages installed by the ChocolateyPackages resource. DSC composite resources are applied in alphabetical order, so this resource runs after ChocolateyPackages, allowing you to express installation order dependencies between packages.
****
