...
:abstract:    {YmlCategory} manages DHCP option values at the scope level.
...
****
Use this resource when you need to set DHCP options (such as DNS servers, domain name, or router addresses) for a specific DHCP scope. Scope-level options override server-level options and apply only to clients within that scope's IP address range. Use DhcpServerOptions for server-wide settings instead.
****
