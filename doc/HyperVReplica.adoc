...
:abstract:    {YmlCategory} configures Hyper-V Replica settings for virtual machine replication to a secondary host.
...
****
Use this resource when you need to set up Hyper-V Replica for disaster recovery. Hyper-V Replica asynchronously replicates virtual machines to a replica server, providing business continuity in case of primary host failure.
****
...
|
| Hashtable[]
| Set of virtual machines to configure for replication.
|
...
.See also
****
* xref:hyperv.adoc[HyperV]
* xref:hypervstate.adoc[HyperVState]
****
