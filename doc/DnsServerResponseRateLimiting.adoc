...
:abstract:    {YmlCategory} configures Response Rate Limiting (RRL) on a Windows DNS server.
...
****
Use this resource when you need to protect your DNS server from denial-of-service attacks via DNS amplification. Response Rate Limiting throttles repetitive responses to the same client, mitigating DNS reflection/amplification attacks while maintaining service for legitimate clients.
****
...
| [[dscyml_dnsserverresponseratelimiting_exceptions, {YmlCategory}/Exceptions]]<<dscyml_dnsserverresponseratelimiting_exceptions_details, Exceptions>>
|
| Hashtable[]
