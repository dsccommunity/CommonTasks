...
:abstract:    {YmlCategory} manages SMB server settings, file shares, and share-level access permissions.
...
****
Use this resource when you need to create file shares, configure share permissions (full access, change, read, no access for specific users/groups), and manage SMB server configuration settings. It can also ensure the shared folders exist on disk before creating the shares.
****
