...
:abstract:    {YmlCategory} runs custom PowerShell script blocks on target nodes using Get/Set/Test patterns.
...
****
Use this resource as a last resort when no dedicated DSC resource exists for what you need to configure. It allows you to run arbitrary PowerShell code with Get, Set, and Test script blocks, following the DSC resource pattern. Prefer purpose-built DSC resources when available, as they provide better idempotency and error handling.
****
