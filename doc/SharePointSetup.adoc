...
| [[dscyml_sharepointsetup_languagepacks, {YmlCategory}/LanguagePacks]]<<dscyml_sharepointsetup_languagepacks_details, LanguagePacks>>
|
| Hashtable[]
| Array of language packs to install, each splatted to the SPInstallLanguagePack DSC resource.
|

|===


[[dscyml_sharepointsetup_languagepacks_details]]
.Attributes of category '<<dscyml_sharepointsetup_languagepacks>>'
[cols="1,1,1,2a,1a" options="header"]
|===
| Parameter
| Attribute
...
| Key
| String
| Specifies the directory where the language pack binary files are located.
|

| Ensure
|
| String
| Specifies whether the language pack should be installed or removed.
| - Present
- Absent
